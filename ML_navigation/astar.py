...
This script implements an Automated Guided Vehicle (AGV) navigation system using the A* search
...
1. Database: Handles loading and updating block locations and the AGV's start position from a CSV
   file.
2. Navigation: Implements the A* search algorithm with both Manhattan and diagonal heuristics for
   pathfinding.
...
class Navigation:
    """
    Provides pathfinding algorithms (A* with Manhattan and diagonal heuristics)
    ...
    def a_star_search(self, start, goal, agv_size, diagonal=False):
        """
        Execute A* search with optional diagonal movement and AGV size consideration.
        ...
        raise ValueError(
            "No valid path found: A* could not reach the goal from the given start."
        )

    def run_a_star(self, start, goal, agv_size, diagonal=False):
        """
        Perform A* search to find the shortest path from start to goal with obstacle and AGV size
        awareness.

        This function implements the A* pathfinding algorithm, which finds the optimal route
        between a start and goal position on a grid. It supports both four-way (cardinal)
        and eight-way (diagonal) movement based on the `diagonal` parameter.

        Args:
            start (tuple): Start position (y, x).
            goal (tuple): Goal position (y, x).
            agv_size (int): AGV size (radius in grid units).
            diagonal (bool): Enable diagonal movement.

        Returns:
            tuple or None: (Path list, exploration heatmap) if path found, else None.
        """
        self.goal = goal
        ...
        ## flag to check for obstalces within agv radius more efficiently
        first_iteration = True

        ## loop until all nodes are explored, meaning path has been found or no valid path exists
        while open_set:
            ...
            if current == self.goal:
                print(
                    f"Total Cost of A Star path with Radius = {agv_size}: {f_score[current]}"
                )
            ...
            for dx, dy in (
                ...
                ## check that move is valid (some diagonal moves collide with obstacles)
                if self.is_valid_move(agv_size, x, y, dx, dy, first_iteration):
                    ...
                    ## if neighbour is not in open set or a shorter path is found, update it
                    if (
    ...
    def reconstruct_path(came_from, current, start):
        ...
        Args:
            came_from (dict): A dictionary that maps each node to its predecessor in the path.
            current (tuple): The current node (goal node initially).
            start (tuple): The start node.

        Returns:
            list: The reconstructed path as a list of (y, x) tuples from start to goal.
        """
        path = []
...
class AGV:
    """
    Plot grid and animate AGV movement. Plot heatmap of cells visited during A* search.
    ...
    def agv_animation(
        ...
        try:
            ...
            print("A* animation saved as agv_animation_astar_search.mp4")
            time_a2 = time.time()
...
def main():
    """
    Main entry point: parses CLI arguments, runs A* search, animates AGV,
    ...
    parser.add_argument(
        ...
        default="True",
        help="Wheather or not diagonal movement for A Star search is allowed. Defaults to 'True'.",
    )
    parser.add_argument(
        "--agv_orientation",
        type=str,
